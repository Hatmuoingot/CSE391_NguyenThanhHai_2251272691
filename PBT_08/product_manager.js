...
const filterProducts = (list, cat, min, max) => 
    list.filter(p => p.category === cat && p.price >= min && p.price <= max);

const sortByPrice = (list, order = "asc") => 
    [...list].sort((a, b) => order === "asc" ? a.price - b.price : b.price - a.price);

const cheapestByCategory = (list) => 
    list.reduce((acc, p) => {
...
const totalInventoryValue = (list) => 
    list.reduce((sum, p) => sum + (p.price * p.stock), 0);

const formatProductList = (list) => 
    list.map(p => ({ name: p.name, formattedPrice: p.price.toLocaleString("vi-VN") + "đ" }));

const averageRating = (list) => 
    list.reduce((sum, p) => sum + p.rating, 0) / list.length;
...
console.log(getInStock(products).length + " sản phẩm còn hàng.");
